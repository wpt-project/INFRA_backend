/**
 * Apply the audit logs table migration (LOGIN-3.12).
 * Run: pnpm --filter @wpt/backend exec tsx src/db/apply-audit-log-migration.ts
 *
 * Executes src/db/migrations/005_audit_logs.sql against DATABASE_URL.
 * Creates the internal `audit_logs` table (admin-only read).
 */
...
// Load .env from backend root (mirrors apply-dashboard-migration.ts).
try {
...
async function main() {
  ...
  console.log("Applying 005_audit_logs.sql ...");
